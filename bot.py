...
async def log_member(u: Update, c: ContextTypes.DEFAULT_TYPE):
    """
    Passive listener: silently records the name + numeric ID of anyone who
    sends a message, so the admin can look up IDs later via /seen.
    No reply is sent; the user sees nothing.
    """
    user = u.effective_user
    if not user:
        return
    state = load_state()
    seen = state.setdefault("seen_users", {})
    full = " ".join(filter(None, [user.first_name, user.last_name])) or "(no name)"
    entry = {
        "id": user.id,
        "name": full,
        "username": user.username or "",
    }
    # Key by id so we keep one entry per person, always up to date
    if seen.get(str(user.id)) != entry:
        seen[str(user.id)] = entry
        save_state(state)

async def cmd_seen(u: Update, c: ContextTypes.DEFAULT_TYPE):
    """
    Admin lookup of everyone the bot has observed, with their numeric IDs.
    Only replies in a PRIVATE chat with the bot (so IDs aren't shown in the
    group). DM the bot and send /seen.
    """
    if u.effective_chat and u.effective_chat.type != "private":
        await u.message.reply_text(
            "🔒 For privacy, send /seen to me in a *private* chat "
            "(tap my name → Message).", parse_mode=ParseMode.MARKDOWN)
        return

    state = load_state()
    seen = state.get("seen_users", {})
    if not seen:
        await u.message.reply_text(
            "No members observed yet. Once people post in the group, "
            "they'll appear here.")
        return

    lines = ["👥 *People the bot has seen*", ""]
    for entry in sorted(seen.values(), key=lambda e: e["name"].lower()):
        uname = f" (@{entry['username']})" if entry["username"] else ""
        lines.append(f"• {entry['name']}{uname}\n   ID: `{entry['id']}`")
    lines += ["", "_Copy the ID of the person you want and put it in the "
              "`BONUS_USER` setting._"]
    await u.message.reply_text("\n".join(lines), parse_mode=ParseMode.MARKDOWN)
...
async def cmd_bonus(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_allowed:
        await u.message.reply_text("🚫 Only the Chosen One may claim bonus points.")
        return
    ...
    if not args:
        await u.message.reply_text(
            f"✨ Your bonus tally stands at *{current}* glorious points.",
            parse_mode=ParseMode.MARKDOWN)
        return
    ...
    await u.message.reply_text(
        ...
        f"🏅 Your all-time bonus tally is now *{new_total}* points.\n"
        f"_(Gloriously meaningless, as ever.)_",
        parse_mode=ParseMode.MARKDOWN)
...
def main():
    ...
    for cmd, fn in [("start", cmd_start), ("help", cmd_start),
                    ...
                    ("bonus", cmd_bonus), ("seen", cmd_seen),
                    ("refresh", cmd_refresh)]:
        app.add_handler(CommandHandler(cmd, fn))

    # Passive listener: log every poster's name + ID (no reply). Runs on any
    # non-command text/message so /seen can reveal IDs without user input.
    app.add_handler(MessageHandler(~filters.COMMAND, log_member), group=1)
